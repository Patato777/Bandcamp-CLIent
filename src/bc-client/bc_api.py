...
class Album(Music):
    def __init__(self, url, json, loaded = True):
        ...
        self.url = url
        self.json = json
        tracks_common_json = {'inAlbum': {'@id': self.json['@id'], '@type': self.json['@type'], 'name': self.json['name']}, 'byArtist': self.json['byArtist'], 'publisher': self.json['publisher'], 'keywords': self.json['keywords']}
        self.tracks = [Track(track['item']['@id'], {**tracks_common_json, **track['item']}, False) for track in self.json['track']['itemListElement']]
...
class Search_item:
    def __init__(self, soup):
        ...
        self.id = self.soup.find('div', class_='heading').text.strip()
...
class Webpage:
    ...
    def __init__(self, url, params=dict()):
        ...
        self.type = self.soup.find('meta', property='og:type').attrs['content'] if self.soup.find('meta', property='og:type') != None else 'search'
