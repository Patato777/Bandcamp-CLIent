...
class Wrapper:
    ...
    def build(self):
        ...
        self.time_bar.set_totaltime(100)
        ...
        self.make_menu([['20','ALBUM','Super Meat Boy! - Digital Special Edition Soundtrack'],['21','TRACK','Dun Dun Dun']]*20)
    ...
    def getcmd(self):
        ...
        if self.cmd in self.commands.keys():
            ...
            self.cmdwin.addstr(0, 0, 'Unknow command', curses.A_STANDOUT)
            self.cmderror = True
        self.cmdwin.refresh()
        self.listener = keyboard.Listener(on_press=self.on_press)
        self.listener.start()
    ...
    def on_press(self, key):
        ...
        if hasattr(self.key, 'char'):
            if self.key.char in self.binds.keys():
                ...
                logging.debug('Key pressed: ' + self.key.char)
...
class ProgBar :
    ...
    def set_totaltime(self, totaltime):
        ...
        self.totalt_str = time.strftime("%H:%M:%S", time.gmtime(totaltime))\
                          if totaltime >= 3600 else time.strftime("%M:%S", time.gmtime(totaltime))
